    ...
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 p-4 md:p-6 lg:h-[calc(100vh-100px)] lg:overflow-hidden animate-in slide-in-from-right-8 duration-300">
          ...
          {isMetadataExpanded && (
            <div className="pt-4 mt-3 border-t border-slate-200 dark:border-white/10 space-y-4 animate-in fade-in slide-in-from-top-2">
              <MetadataField label="Specialty Domain">
                <select value={selectedSia} onChange={(e) => setSelectedSia(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white outline-none">
                  {SPECIALTIES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </MetadataField>
              <div className="grid grid-cols-2 gap-3">
                <MetadataField label="Level">
                  <select value={trainingLevel} onChange={(e) => setTrainingLevel(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white outline-none">
      ...
      <div className="hidden lg:flex lg:col-span-4 flex-col gap-6 overflow-y-auto pr-2">
        ...
        <GlassCard className="p-8">
          <h2 className="text-xl font-semibold text-slate-900 dark:text-white/90 mb-6 flex items-center gap-2">
            <Clipboard className="text-indigo-600" size={24} /> CBD Assessment
          </h2>
          <div className="space-y-6">
            <MetadataField label="Specialty Domain">
              <select value={selectedSia} onChange={(e) => setSelectedSia(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500/50">
                {SPECIALTIES.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </MetadataField>
            <MetadataField label="Level">
              <select value={trainingLevel} onChange={(e) => setTrainingLevel(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500/50">
                {[1,2,3,4].map(l => <option key={l} value={l}>Level {l}</option>)}
              </select>
            </MetadataField>
            ...
            <MetadataField label="Supervisor / Assessor">
              <div className="space-y-2">
                <input type="text" placeholder="Assessor Name" value={assessorName} onChange={(e) => setAssessorName(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500/50" />
                <input type="email" placeholder="Assessor Email" value={assessorEmail} onChange={(e) => setAssessorEmail(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500/50" />
              </div>
            </MetadataField>
            ...
            <div className="pt-4 flex flex-col gap-3">
              <button className="w-full py-3 rounded-xl bg-indigo-600 text-white text-sm font-semibold shadow-lg shadow-indigo-600/20">Submit CBD</button>
          ...
          {activeSection === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <GlassCard className="p-4 lg:p-6 space-y-8">
                <RadioGroup label="Overall assessment of this case‑based discussion" options={["Major concerns", "Minor concerns", "Meets expectations"]} value={overallAssessment} onChange={setOverallAssessment} />
                ...
                <div><label className="text-[10px] uppercase tracking-widest text-slate-400 dark:text-white/30 font-bold mb-2 block">Agreed action plan</label><textarea value={actionPlan} onChange={(e) => setActionPlan(e.target.value)} className="w-full min-h-[80px] bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm" /></div>
              </GlassCard>
            </div>
          )}
        </div>
