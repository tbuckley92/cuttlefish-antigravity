-- Migration: Fix Notification Trigger for Evidence Sign-Off
-- This notifies the Trainee when their evidence is Signed Off
-- Fixed to use correct status value 'COMPLETE' instead of 'SignedOff'
...
BEGIN
  -- Only proceed if status is 'COMPLETE' (SignedOff in the application)
  -- And either it's a new record (unlikely to be COMPLETE immediately but possible) OR the status changed to 'COMPLETE'
  IF (TG_OP = 'INSERT' AND NEW.status = 'COMPLETE') OR
     (TG_OP = 'UPDATE' AND NEW.status = 'COMPLETE' AND OLD.status <> 'COMPLETE') THEN
